...
class WebSockets {
  connection(client) {
    ...
    client.on("subscribe_read_at", (roomId) => {
      client.join(roomId);
    });
...
function ioUpdateByRoomId(rooms, taskGroupType, action, data, currentUserId) {
  rooms.map(async (roomId) => {
    ...
    members.map((userId) => {
      ...
      if (action === "new message" && userId === data.postedByUser) {
        return;
      }
...
function ioUpdateToMessageSender(postedByUser, action, data) {
  const socketId = getUserSocketIdByUserId(postedByUser);
